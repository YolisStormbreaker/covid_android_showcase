...
import com.yolisstorm.library_common.extensions.second
...
suspend fun calculateMidpointBetweenPoints(listOfPoints: List<LatLng>): LatLng =
	if (listOfPoints.size > 2) {
		...
		findCentreOfPolygon(list) ?: LatLng(57.856402, 33.528651)
	} else {
		calculateMidpointBetweenTwoPoints(listOfPoints.first(), listOfPoints.second())
	}
...
fun checkIfPolygonsContainsPoint(
	...
	polygons.forEach {
		if (isPolygonContainPoint(point, it))
			return true
	}
	return false
}
